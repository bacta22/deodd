...
@Slf4j
@RegisterHandler(protocol = VhProtocol.HTTP, method = VhHttpMethod.GET, endpoint = "/users/ref/check")
public class CheckUserValidForReferralOrNotHandler extends AbstractHandler<Boolean> {
    @Inject
    UserRepository userRepository;

    @Override
    protected CompletionStage<BefHttpResponse<Boolean>> doHandle(VhHttpRequest request) {
        String address = request.getQueryParam("address");
        return userRepository.checkUserValidForReferral(address)
