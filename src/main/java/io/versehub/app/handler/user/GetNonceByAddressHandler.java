...
@Slf4j
@RegisterHandler(protocol = VhProtocol.HTTP, method = VhHttpMethod.GET, endpoint = "/users/nonce/address")
public class GetNonceByAddressHandler extends AbstractHandler<Integer> {

    @Inject
    UserRepository userRepository;

    @Override
    protected CompletionStage<BefHttpResponse<Integer>> doHandle(VhHttpRequest request) {
        String address = request.getQueryParam("address");
        return userRepository.getNonceByAddress(address)
